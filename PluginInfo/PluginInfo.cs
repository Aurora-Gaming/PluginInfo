...
public class PluginInfo : TerrariaPlugin
{
	...
	{
		bool evaluator(TerrariaPlugin plugin) => !debugOnly || GetAssemblyConfiguration(plugin.GetType().Assembly) == "Debug";
	...
	void ListPlugins(List<TerrariaPlugin> plugins, TSPlayer receiver, List<string> args)
	{
		...
		else if (receiver.RealPlayer)
		{
			...
			if (!PaginationTools.TryParsePageNumber(args, 1, receiver, out int pageNum))
				return;
	...
	{
		if (asm.GetCustomAttributes(typeof(DebuggableAttribute), false).FirstOrDefault() is DebuggableAttribute attrib)
